...
const StudyPlanForm = ({ form, onSubmit, isLoading }: { form: any, onSubmit: (data: any) => Promise<void>, isLoading: boolean }) => (
  <>
    <div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <CardContent className="grid gap-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              <FormField control={form.control} name="weeks" render={({ field }) => (<FormItem><FormLabel>Total Weeks</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)} />
              ...
              <FormField control={form.control} name="hoursPerDay" render={({ field }) => (<FormItem><FormLabel>Hours per Day</FormLabel><FormControl><Input type="number" step="0.5" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="startDate" render={({ field }) => (
                <FormItem className="flex flex-col pt-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={(date) => date < new Date(new Date().setHours(0,0,0,0))} initialFocus/></PopoverContent>
                  </Popover>
                  <FormMessage />
                </FormItem>
              )} />
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <FormField control={form.control} name="pace" render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select a pace" /></SelectTrigger></FormControl><SelectContent><SelectItem value="slow">Slow & Steady</SelectItem><SelectItem value="medium">Medium</SelectItem><SelectItem value="fast">Fast-paced</SelectItem></SelectContent></Select>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
            <FormField control={form.control} name="interests" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="e.g., I'm a complete beginner. I want to focus on practical, hands-on examples." {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
          </CardContent>
          <CardFooter>
            <Button className="w-full" type="submit" disabled={isLoading}>
              {isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Generating Plan...</>) : (<><BrainCircuit className="mr-2 h-4 w-4" /> Generate Plan</>)}
            </Button>
          </CardFooter>
        </form>
      </Form>
...
const StudyPlanView = ({ studyPlan, onResetPlan, onSendToPlans, onTaskToggle, progress, weeklyGroupedTasks }: { studyPlan: StudyPlan, onResetPlan: () => void, onSendToPlans: () => void, onTaskToggle: (id: string, checked: boolean) => void, progress: any, weeklyGroupedTasks: any[] }) => (
  <>
    <div>
      <Button variant="ghost" onClick={onResetPlan} className="mb-4 -ml-4"><ArrowLeft className="mr-2 h-4 w-4"/>Create a New Plan</Button>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="flex justify-between text-sm mb-1"><p className="font-medium">Task Completion</p><p className="text-muted-foreground">{progress.completed} of {progress.total} tasks</p></div>
          <Progress value={progress.percentage} className="h-2"/>
        </div>
        <div>
          <div className="flex justify-between text-sm mb-1"><p className="font-medium">Time Studied</p><p className="text-muted-foreground">{progress.hoursStudied} / {progress.totalHours} hours</p></div>
          <Progress value={(progress.totalHours > 0 ? (progress.hoursStudied / progress.totalHours) * 100 : 0)} className="h-2"/>
        </div>
      </CardContent>
      <CardFooter className="gap-2 flex-wrap">
        <Button onClick={onSendToPlans}><Send className="mr-2 h-4 w-4" />Send to Study Plans</Button>
        <Button variant="destructive" onClick={onResetPlan}><Trash2 className="mr-2 h-4 w-4"/>Discard Plan</Button>
      </CardFooter>
    </Card>
        ...
        <AccordionItem value={`week-${week.week}`} key={week.week}>
          <AccordionTrigger>
            <div className="flex flex-col items-start">
              ...
              <p className="text-sm text-muted-foreground font-normal">{week.tasks.filter((t: StudyTask) => t.completed).length} / {week.tasks.length} tasks completed</p>
            </div>
          </AccordionTrigger>
            ...
            {week.tasks.map((task: StudyTask) => (
              <Card key={task.id} className={task.completed ? 'bg-muted/50' : ''}>
                <CardContent className="p-4 flex items-start gap-4">
                  <Checkbox id={task.id} checked={task.completed} onCheckedChange={(checked) => onTaskToggle(task.id, !!checked)} className="mt-1" />
...
export default function StudyPlanPage() {
  ...
  const handleGeneratePlan = async (data: z.infer<typeof planFormSchema>) => {
    ...
    const aiInput: StudyPlanInput = { subject: data.subject, duration, details, startDate: format(data.startDate, 'yyyy-MM-dd') };
  ...
  const handleSendToPlans = () => {
    if (!studyPlan) return;
    addPlan(studyPlan);
    toast({ title: "Plan Saved!", description: `"${studyPlan.title}" has been added to your Study Plans.` });
    handleResetPlan();
  };
  ...
  const weeklyGroupedTasks = useMemo(() => {
    ...
    studyPlan.tasks.forEach(task => {
      ...
      } catch (error) { console.warn(`Skipping task with invalid date:`, task); }
    });
    return Object.keys(groupedByWeek).sort((a, b) => a.localeCompare(b)).map((weekKey, index) => ({
      week: index + 1,
    ...
    <div className="flex flex-col gap-6">
      {studyPlan ? (
        <StudyPlanView 
          ...
          progress={progress}
          weeklyGroupedTasks={weeklyGroupedTasks}
        />
      ) : (
        <StudyPlanForm 
          form={form}
